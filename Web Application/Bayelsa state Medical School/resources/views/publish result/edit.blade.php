...
<h1>Dashboard  &nbsp;&nbsp;&nbsp;&nbsp;<span class="welcome-heading-page">{{ Auth::user()->name }} </span></h1>  <span class="mainbody" style="margin-left:1px;">Publish Result  Page</span>
...
<div id="error-page" style="text-align:center; font-weight:bold;"> </div>
  ...
  <div class="form-group{{ $errors->has('optionA') ? ' has-error' : '' }}"  >
        ...
        <div class="col-md-9" >
          <input id="optionA" type="date" class="form-control" name="optionA" value="{!!$query[0]->date!!}" required autofocus autocomplete="off">
  <input id="acdId" type="hidden" class="form-control" name="acdId" value="{!!$query[0]->acdId !!}" required autofocus autocomplete="off" >
    
    <input id="oldDate" type="hidden" class="form-control" name="oldDate" value="{!!$query[0]->date!!}" required autofocus autocomplete="off">
    
    
    <input id="userId" type="hidden" class="form-control" name="userId" value="{{ Auth::user()->id }}" required autofocus autocomplete="off" >
    
    <input id="id" type="hidden" class="form-control" name="id" value="{!!$query[0]->pubId!!}" required autofocus autocomplete="off" >
...
var dataArray= JSON.stringify(dtArray);
     ...
     $.ajax({ 
	  dataType:"json", 
	    type:"POST",
         url:'/edit-publish-result-date/{{$query[0]->pubId}}',
