...
<div id="error-page" style="text-align:center; font-weight:bold;"> </div>
  ...
  <div class="form-group{{ $errors->has('optionA') ? ' has-error' : '' }}"  >
          <label for="optionA" class="col-md-4 control-label " style="margin-top:15px; width:70px; " >Level:* &nbsp;</label>
...
$(function() {
 ...
 $("#butSubmit").click(function(ev) {
	...
	document.getElementById('error-page').style.height="auto";
	 ...
	 if(document.getElementById('optionA').value =='')
	{
		
		//document.getElementById('error-page').style.height="25px";
	
		var errors= '<span class=rstclass style=color:#C63A26;> &nbsp;&nbsp; Please Enter Level is Require !! &nbsp;</span>';
...
var dataArray= JSON.stringify(dtArray);
     ...
     $.ajax({  
	    type:"POST",
         url:"create_level",
         data:{"_token":$('#signup-token').val(),"level":$("#optionA").val()},
		  dataType:"JSON",
		 processData:"false",
