...
Public Class frmincome
    ...
    Private Sub butEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butEdit.Click
        Try
            ...
            If ComboBox1.Text = "" Then
                ...
                MsgBox("Payment Type can not be Empty", MsgBoxStyle.Information, "Information")
                Exit Sub
            End If
            ...
            sqlString = "INSERT INTO  income(dpaydate,vYear,vMonth,vUser,vName,vdescrib,vPayment_Type,mAmount) VALUES(" & _
            "'" & dateapp & "'," & _
             "'" & dYear & "'," & _
              "'" & dMonth1 & "'," & _
            "'" & UserN & "'," & _
            ...
            sqlString = "UPDATE  [acct] SET " & _
              "[mincome] = [mincome] + " & Me.TextBox1.Text & " "
    ...
    Private Sub frmincome_Activated(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Activated
        Me.Left = 0
        Me.Top = 100
        Me.Width = 694
        Me.Height = 230
    ...
    Private Sub frmincome_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Click
        Me.BringToFront()
    End Sub
    ...
    Private vSerial, dDate As String

    Private Sub serail()
        On Error Resume Next
        Dim dts, dts1 As Integer
        Dim DATEPAY1, DATEPAY, dpaydate As Date

        DATEPAY = CDate(DateTimePicker2.Text)
        Dim lenght As Integer
        lenght = CInt(DATEPAY.Day)

        dts = CInt(DATEPAY.Day)

        Call conns()
        str_sql_user_select = "select max(id )AS  [Studentid] from INCOME  "
        comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
        myreader = comUserSelect.ExecuteReader()

        While myreader.Read
            dts1 = CInt(myreader("Studentid"))
        End While
        mycon.Close()
        myreader.Close()

        Call conns()
        str_sql_user_select = "SELECT  COUNT(*) AS [Account]  FROM  [INCOME] "
        comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
        myreader = comUserSelect.ExecuteReader()
        Dim rowAcct As Integer

        While myreader.Read
            rowAcct = myreader("Account")
        End While
        myreader.Close()
        mycon.Close()
        If rowAcct = 0 Then
            dDate = CStr(DATEPAY & "" & "-01")
            vSerial = dDate
            Exit Sub
        End If
        ...
        If rowAcct <> 0 Then
            ...
            str_sql_user_select = "select substring(vRC_No,11,14)AS  [Studentid] from INCOME  where id=" & dts1 & " "

            comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
            myreader = comUserSelect.ExecuteReader()

            While myreader.Read
                newdate = CInt(myreader("Studentid"))
            End While
            mycon.Close()
            myreader.Close()
            ...
            If lenght <= 9 Then
                str_sql_user_select = "select substring(vRC_No,0,9)AS  [Studentid] from INCOME   where id=" & dts1 & " "
            End If

            If lenght > 9 Then
                str_sql_user_select = "select substring(vRC_No,0,10)AS  [Studentid] from INCOME   where id=" & dts1 & " "
            End If

            'str_sql_user_select = "select substring(vRC_No,0,9)AS  [Studentid] from payFees where id=" & dts1 & "  "
            comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
            myreader = comUserSelect.ExecuteReader()

            While myreader.Read
                DATEPAY1 = CDate(myreader("Studentid"))
        ...
        On Error Resume Next
        If rowAcct <> 0 And DATEPAY1 = DATEPAY Then
            Call conns()

            str_sql_user_select = "select max(substring(vRC_No,11,14))AS  [Studentid] from INCOME  where dpayDate='" & dpaydate & "'"
            comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
            myreader = comUserSelect.ExecuteReader()
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        Try
            ...
            Call conns()
            str_sql_user_select = "SELECT COUNT(*) AS [RecordCount] FROM   Income  where  vName= '" & TextBox3.Text.Trim & "'  and  id =" & dts1 & "  "
            ' comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
            'myreader = comUserSelect.ExecuteReader()
            comUserSelectS = New OleDbCommand(str_sql_user_select, strconss)
            myAccessReader = comUserSelectS.ExecuteReader()
            ...
            If numbs_pay = 0 Then
                ...
                str_sql_user_select = "SELECT *  FROM Income WHERE vName ='" & TextBox3.Text.Trim & "'  and    id =" & dts1 & "  "
            ...
            If TextBox1.Text <> "" And TextBox2.Text <> "" Then
                e.Graphics.DrawString("OTHER INCOME PAYMENT DETAILS.", headfont, Brushes.Black, X1 + 70, Y + 70)
                ...
                For Each i As String In Me.ListItems.Items(itm).ToString
                    ...
                    While myAccessReader.Read
                        ...
                        e.Graphics.DrawString("INCOME : ", tablefont, Brushes.Black, X1 + 280, Y + 110)
