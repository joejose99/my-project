...
Public Class frmPledgeRedeem

    Private Sub frmPledgeRedeem_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.Left = 0
        Me.Top = 100
    ...
    Private Sub TextBox6_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs) Handles TextBox6.DoubleClick
        TextBox6.Text = ""
        frmPledgePuup.MdiParent = mdiChurch
        frmPledgePuup.Show()
        frmPledgePuup.BringToFront()
        frmPledgePuup.WindowState = FormWindowState.Normal
    ...
    Public Property AddMebId() As String
        ...
        Set(ByVal value As String)
            ...
            Try
                ...
                str_sql_user_select = "SELECT * FROM  FinPledge where vMemberId= '" & TextBox6.Text & "'"
                ...
                While myAccessReader.Read()
                    ...
                    TextBox5.Text = myAccessReader("vSurname")
                    TextBox3.Text = myAccessReader("vFname")

                    TextBox7.Text = myAccessReader("vPhoneNo")
                    amnts = myAccessReader("mAmount")
                    TextBox1.Text = myAccessReader("mAmount")
                    TextBox2.Text = myAccessReader("vPurpose")
    ...
    Private Sub generateId()
        Try
            ...
            Call conns()
            str_sql_user_select = "SELECT COUNT(vMemberId) AS [RecordCount] FROM  [RedPledge] where vAuto ='" & status & "'  "
            ...
            If rows = 0 Then
                newdate1 = "PL" & year & "00" & "1"
                TextBox6.Text = Trim(newdate1)
            End If
            ...
            If rows <> 0 Then
                ...
                Dim maxId As String = ""
                Call conns()
                str_sql_user_select = "select max(id)AS  [Studentid] from RedPledge where vAuto ='" & status & "'  "
                'comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
                'myreader = comUserSelect.ExecuteReader()
                ...
                Call conns()
                str_sql_user_select = "select  [dpaydate] from RedPledge where vAuto ='" & status & "' and id =" & maxId & "  "
                'comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
                'myreader = comUserSelect.ExecuteReader()
                ...
                'While myreader.Read
                While myAccessReader.Read
                    newdate = Trim(myAccessReader("dpaydate")) & ""
    ...
    Private Sub butEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles butEdit.Click
        ...
        Try
            ...
            Dim sqlString As String = ""
            sqlString = "INSERT INTO  RedPledge(dpaydate,vYear,vMonth,vUser,vSurname,vFName,vPhoneNo,vMemberId,vPurpose,vAuto,mAmount) VALUES(" & _
            "'" & dateapp & "'," & _
             "'" & dYear & "'," & _
              "'" & dMonth1 & "'," & _
                "'" & UserN & "'," & _
 ...
 Call conns()
            ...
            sqlString = "UPDATE  [acct] SET " & _
             "[mPledges] = [mPledges] + " & Me.TextBox1.Text & " "
            ...
            If Trim(_stid) = TextBox6.Text.Trim And amnts.Trim = TextBox1.Text.Trim Then
                Call conns()
                str_sql_user_select = "Delete from [FinPledge] where vMemberId='" & _stid & "' and mAmount =" & amnts & ""
    ...
    Public Sub PrintBIll()
        ...
        With PrintDocument1.DefaultPageSettings
            ...
            PHi = PSize * 20 + 350
            Ps = New Printing.PaperSize("Other Expenses Payment Details", 800, 1000)
            .Margins.Top = 15
            .Margins.Bottom = 20
            .PaperSize = Ps
        End With
    ...
    Private Sub PrintDocument1_PrintPage(ByVal sender As System.Object, ByVal e As System.Drawing.Printing.PrintPageEventArgs) Handles PrintDocument1.PrintPage
        ...
        Try
            ...
            Dim dts1 As Integer
            Call conns()
            str_sql_user_select = "select max(id )AS  [Studentid] from RedPledge  "
            ' comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
            'myreader = comUserSelect.ExecuteReader()
            ...
            Call conns()
            str_sql_user_select = "SELECT COUNT(*) AS [RecordCount] FROM   RedPledge  where  vFName= '" & TextBox3.Text.Trim & "'  and  id =" & dts1 & "  "
            ' comUserSelect = New SqlClient.SqlCommand(str_sql_user_select, mycon)
            'myreader = comUserSelect.ExecuteReader()
            comUserSelectS = New OleDbCommand(str_sql_user_select, strconss)
            myAccessReader = comUserSelectS.ExecuteReader()
            ...
            If numbs_pay = 0 Then
                ...
                str_sql_user_select = "SELECT *  FROM RedPledge WHERE vFName ='" & TextBox3.Text.Trim & "'  and    id =" & dts1 & "  "
            ...
            If TextBox1.Text <> "" And TextBox2.Text <> "" Then
                e.Graphics.DrawString(" REDEEM PLEDGE PAYMENT DETAILS.", headfont, Brushes.Black, X1 + 70, Y + 70)
                ...
                For Each i As String In Me.ListItems.Items(itm).ToString
                    ...
                    While myAccessReader.Read
                        ...
                        e.Graphics.DrawString("SURNAME: ", tablefont, Brushes.Black, X1 + 15, Y + 110)
                        e.Graphics.DrawString(myAccessReader("vSurName"), titlefont, Brushes.Black, X1 + 100, Y + 110)

                        e.Graphics.DrawString("First Name: ", tablefont, Brushes.Black, X1 + 200, Y + 110)
                        e.Graphics.DrawString(myAccessReader("vFName"), titlefont, Brushes.Black, X1 + 280, Y + 110)
                        ...
                        e.Graphics.DrawString(myAccessReader("dpaydate"), titlefont, Brushes.Black, X1 + 100, Y + 180)





                        e.Graphics.DrawString("PLEDGE : ", tablefont, Brushes.Black, X1 + 440, Y + 110)
                        e.Graphics.DrawString(Format(Val(myAccessReader("mAmount"))), titlefont, Brushes.Black, X1 + 500, Y + 110)
                        ...
                        End If
                        e.Graphics.DrawString("PURPOSE : ", tablefont, Brushes.Black, X1 + 240, Y + 180)
                        e.Graphics.DrawString(myAccessReader("vPurpose"), titlefont, Brushes.Black, CInt(X1) + 440, CInt(Y) + 180)
